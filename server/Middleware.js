...
async function authenticateAndValidateUser(req, res, next) {
    ...
    try {
        ...
        const user = await db.findById(User, decoded.user_id);  // Fetch user from DB
        if (!user) return res.status(400).json({ success: false, message: 'User not found!' });

        const isMatch = await bcrypt.compare(decoded.password, user.password);  // Compare plain password from JWT and hashed from DB
